...
const patchShoppingCart = (req: Request, res: Response, next: NextFunction) => {
    ...
    ShoppingCart.find({name, userId: user._id})
        ...
        .then((response: IShoppingCart[]) => {
            ...
            return cart.updateOne({name: newName || name, products: products || cart.products})
                .then((response: any) => {
...
const deleteShoppingCart = (req: Request, res: Response, next: NextFunction) => {
    const {name} = req.body;
    const {user} = res.locals;
...
const aggregateProductQuantityByName = (req: Request, res: Response, next: NextFunction) => {
